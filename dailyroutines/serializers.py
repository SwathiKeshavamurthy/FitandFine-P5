...
class DailyRoutineSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = DailyRoutine
        fields = [
            'id', 'owner', 'person_name', 'date', 'wake_up_time',
            'breakfast_time', 'lunch_time', 'dinner_time',
            'total_calorie_intake', 'water_intake', 'sleep_time',
            'workout_minutes', 'junk', 'mood', 'created_at',
            'updated_at'
        ]
    ...
    def validate_total_calorie_intake(self, value):
        if value < 0:
            raise serializers.ValidationError(
                "Total calorie intake must be non-negative."
            )
        return value
    ...
    def validate_water_intake(self, value):
        if value < 0:
            raise serializers.ValidationError(
                "Water intake must be non-negative."
            )
        return value
    ...
    def validate_workout_minutes(self, value):
        if value < 0:
            raise serializers.ValidationError(
                "Workout minutes must be non-negative."
            )
        return value
